...
function sortBubble(array) {
  ...
  for (let i = 0; i < len; i++) {
    for (let j = 0; j < len; j++) {
      // jika nilai dupe[j] lebih besar daripada nilai dupe[j + 1], maka tukar
      if (dupe[j] > dupe[j + 1]) {
        let tempDupe = dupe[j]; // tampung sementara nilai dupe[j]
        dupe[j] = dupe[j + 1]; // tukar nilai dupe[j] dengan nilai dupe[j + 1]
        dupe[j + 1] = tempDupe; // tukar nilai dupe[j + 1] dengan nilai yang sudah ditampung sebelumnya di tempDupe
      }
    }
  }
